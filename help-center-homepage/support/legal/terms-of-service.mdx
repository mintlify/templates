---
title: "Terms of service"
sidebarTitle: "Terms of service"
description: "Demo terms of service for the Banyan help center template."
---

## Acceptance

By using Banyan in this demo, you agree to illustrative terms meant for layout and navigation only.

## Service changes

Features described here may change without notice in a template environment.

## Liability

This text is not legal advice and is not a binding agreement—consult your legal team.